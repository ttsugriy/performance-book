...
(This is a simplification—MATLAB was also influenced by Fortran's array operations and BLAS, and NumPy draws from multiple traditions. But APL, and particularly Iverson's insight that notation shapes thought [@iverson1979notation], is the key intellectual ancestor.)
...
## A Simpler Example: Parallel Reduction

The FlashAttention case study above is spectacular but complex. The same pattern appears in much simpler form in **parallel reduction** — arguably the most universal building block in high-performance computing.
...
**The array version:**
```python
total = data.sum()  # or np.sum(data)
```

The loop version hides structure. The array version reveals it: this is a **reduction** — combining elements with an associative operator. Because addition is associative, the system can:

- **Chunk** the data and compute partial sums on different cores
- **Vectorize** using SIMD (add 8 numbers in one instruction)
- **Tree-reduce** partial results in O(log n) steps

This is the same pattern as FlashAttention (associative state combination), but stripped to its essence. Throughout this book, you'll see it again and again: **once you see the structure, the optimization follows**.

::: {.callout-note}
## Three Running Examples

This book uses three main examples to illustrate the framework:

1. **Parallel reduction** (Associativity + Parallelism): The simplest case — appears in summation, softmax, scanning, and distributed gradient sync
2. **FlashAttention** (Associativity + Locality): The flagship — shows how algebraic structure enables algorithmic transformation
3. **Depthwise separable convolution** (Separability + Symmetry): Shows how factoring and symmetry combine to reduce parameters by orders of magnitude

Each exercises different properties and appears at different scales of complexity.
:::
