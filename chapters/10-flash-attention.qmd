...
This is where the associativity insight from the Associativity chapter becomes crucial.
...
Total HBM accesses: O(n²d²/M) where M is SRAM size — Q is loaded once,
but K and V are loaded once per Q-tile (the inner loop streams over K/V
for each Q-tile). With tile size √M, this is much less than O(n²) but
more than O(nd). For typical values (n=2048, d=64, M=100KB), the actual
traffic is ~10× less than standard attention.
...
FlashAttention-2 was architecture-agnostic. FlashAttention-3 exploits Hopper's unique features and can deliver up to ~1.5-2× speedup on H100-sized workloads (shape- and kernel-dependent).
...
Hopper's tensor cores offer roughly 2× the FP8 peak of FP16 for dense math, depending on kernel shape and sparsity mode.
...
**Accuracy:** Typically close to FP16 when scaling is tuned, but varies by model and data distribution.
...
These numbers are from a single benchmark configuration (H100, seq 8192, head dim 128). Actual results vary with sequence length, head dimension, and kernel tuning.
...
**FlashAttention Version Selection:**

- **H100/H200 (Hopper)**: Use FA3
  - Need FP16 accuracy: FA3 FP16
  - Can tolerate FP8: FA3 FP8 (2x faster)
- **A100 (Ampere)**: Use FA2
- **Older/Non-NVIDIA**: Use FA1 or PyTorch SDPA
:::
...
**Memory Hierarchy**: FlashAttention is fundamentally about keeping data in fast memory (SRAM) instead of slow memory (HBM).

**Bandwidth**: The algorithm is faster despite more FLOPs because memory bandwidth, not compute, is the bottleneck.

**Associativity**: The (max, sum, output) state forms a monoid—the mathematical foundation for chunking.

## Property Audit

| Property | Applies? | How it's exploited |
|----------|----------|-------------------|
| **Associativity** | **Primary** | Online softmax: the (max, sum, weighted_sum) state has an associative combine operation, enabling chunked computation |
| **Separability** | No | Attention is fundamentally a full interaction (each query attends to all keys) |
| **Locality** | **Primary** | Tiling Q/K/V into SRAM-sized blocks eliminates O(n²) HBM traffic |
| **Sparsity** | No | Standard FlashAttention computes exact (dense) attention |
| **Redundancy** | No | Exact computation, no approximation |
| **Symmetry** | No | Attention is asymmetric (Q→K, not K→Q) |

**Dominant properties**: Associativity (enables the algorithm) + Locality (enables the speedup). This is the canonical example of Tier 1 + Tier 2 cooperation.
