...
A matrix is stored in row-major order (C layout). You iterate over it in two ways:
(a) Row-by-row: `for i in range(n): for j in range(n): access(M[i][j])`
(b) Column-by-column: `for j in range(n): for i in range(n): access(M[i][j])`
