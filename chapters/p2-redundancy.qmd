...
**Memory bandwidth** (Chapter 2): Quantization from FP16 to INT4 reduces memory traffic by 4×. For memory-bound operations (most LLM inference), this translates directly to 4× throughput improvement.
...
**Cache hierarchy** (Chapter 1): KV cache fits in GPU SRAM for shorter sequences, enabling fast access. When the cache spills to HBM, latency increases—making cache size reduction via quantization or MQA critical.
