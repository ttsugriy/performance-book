...
This is exactly how FlashAttention works.

Standard attention:

```
Q, K, V = query, key, value matrices
S = Q @ K.T           # Attention scores (O(n²) memory!)
P = softmax(S)        # Row-wise softmax
O = P @ V             # Output
```

The problem: `S` is n × n. For sequence length 32K, that's 4 GB per head, per layer, per batch element.

FlashAttention's insight: extend the streaming softmax to track output state too.

The state becomes `(max, sum, output)`:

```python
def flash_attention_block(Q_block, K, V, state=None):
    """Process one block of queries against all keys/values."""
    if state is None:
        m = float('-inf')
        s = 0.0
        o = np.zeros_like(Q_block @ V[:1].T)
    else:
        m, s, o = state
    ...
    for i, row_scores in enumerate(scores):
        ...
        if row_max > m[i]:
            # Max changed: rescale output and sum
...
def flash_attention(Q, K, V, block_size=64):
    """Compute attention without materializing the full attention matrix."""
    n = Q.shape[0]
    state = None
    ...
    for j in range(0, n, block_size):
        ...
        state = flash_attention_block(Q, K_block, V_block, state)

    m, s, o = state
    return o / s[:, None]  # Normalize by softmax denominator
```
...
FlashAttention:
  Q, K, V:      O(n × d)
  State (m,s,o): O(n × d)
  Per-block:    O(block_size × d)
  Total:        O(n × d)

Reduction: O(n²) → O(n)
```

For n = 32,768 and d = 128:
...
## Connection to FlashAttention Investigation

Chapter 11 walks through the full FlashAttention derivation in detail, including:

- The tiling strategy (how to choose block sizes)
- The backward pass (gradients also use streaming)
