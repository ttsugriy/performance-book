...
::: {.chapter-opener}
An H100 GPU can perform about 1,000 trillion operations per second for dense FP16. It can move about 3.35 trillion bytes per second from memory. That's roughly 300 operations per byte. FP8 peaks are higher, which raises the ridge point. If your algorithm does far fewer than ~300 operations per byte, you're not computing—you're waiting.
...
| Ratio | 295 FLOPS/byte |

What does this mean? For every byte you move from memory, you need to do about 300 operations to keep the compute units busy (for dense FP16). If you do fewer, the compute units sit idle waiting for data.
...
For an H100: 989 TFLOPS / 3.35 TB/s ≈ 295 FLOPS/byte.

For operations with intensity below 295: you're memory-bound. Move data faster.

For operations with intensity above 295: you're compute-bound. Use better algorithms.
...
**Operations**:
- QK^T: 2n²d FLOPs
- Softmax: ~5n² FLOPs (exp, sum, divide)
- Multiply by V: 2n²d FLOPs

Total: ~4n²d FLOPs

**Bytes moved** (naive implementation):
- Read Q, K, V: 3nd × 2 bytes (FP16)
- Write attention matrix: n² × 2 bytes
- Read attention matrix: n² × 2 bytes
- Write output: nd × 2 bytes
...
That's well below the H100's ridge point of 295. **Attention is memory-bound.**
...
- Williams et al., ["Roofline: An Insightful Visual Performance Model"](https://people.eecs.berkeley.edu/~kubitron/cs252/handouts/papers/RooflineVyNoYellow.pdf) — The original paper
- NVIDIA, [CUDA Best Practices Guide](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/) — Memory optimization for GPUs
- Intel, [Advisor Roofline Analysis](https://www.intel.com/content/www/us/en/developer/articles/guide/intel-advisor-roofline.html) — Tool for automatic roofline analysis
