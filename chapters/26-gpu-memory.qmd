...
::: {.callout-note}
## Property Spotlight: Redundancy + Reversibility

GPU memory optimization exploits **redundancy** (activation checkpointing discards activations that can be recomputed) and **reversibility** (reversible layers enable recomputation from outputs). Memory-efficient attention (FlashAttention) exploits **locality** by keeping working data in SRAM rather than materializing O(n²) attention matrices in HBM.
:::
...
Optimizer state:        56.0 GB (FP32 Adam momentum + variance)
Activations:            ~8.0 GB (depends on checkpointing)
Temporary buffers:      ~2.0 GB (cuBLAS workspace, etc.)
─────────────────────────────────────────────────────────
Total:                  ~94 GB

A100 80GB: Doesn't fit without optimizer sharding or 8-bit optimizers
...
Step 1: Allocate A (2GB), B (3GB), C (2GB)
Memory: [A:2GB][B:3GB][C:2GB][free:1GB]

Step 2: Free A and B
Memory: [free:2GB][free:3GB][C:2GB][free:1GB]

Step 3: Try to allocate D (5GB)
Problem: Have 6GB free, but no contiguous 5GB block!
(largest contiguous region is 3GB)
...
# Better: In-place operations (inference only!)
y = x.add_(1).mul_(2)  # No intermediate allocation
# WARNING: In-place ops can corrupt autograd during training.
# Only use in-place ops in inference or no-grad contexts.
...
Notebook support for this chapter is in progress. For now, run the memory experiments locally and measure behavior on your hardware.
:::
