...
Model fits in GPU with batch size 1?
  ├─ Yes → Try gradient checkpointing first
  │         Still OOM? → Try smaller batch + gradient accumulation
  │         Still OOM? → CPU offload optimizer
  │
  └─ No → Need offloading
          ├─ Optimizer-only offload (ZeRO-2 + offload)
          │   ~2x slowdown, fits 2x larger model
          │
          ├─ Full offload (ZeRO-3 + offload)
          │   ~5x slowdown, fits 4x+ larger model
          │
          └─ NVMe offload
              ~10x+ slowdown, fits huge models
```
