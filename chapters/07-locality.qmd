...
In Memory Hierarchy, we saw the memory hierarchy:
...
This is why FlashAttention (from the FlashAttention chapter) uses tiling—to restore locality to attention.
...
We've now covered the six fundamental properties that make efficient algorithms possible:

1. **Associativity**: Enables parallel and streaming computation (from Associativity)
2. **Separability**: Enables low-rank approximations and factorization (from Factoring)
3. **Sparsity**: Enables conditional computation and skipping (from Skipping)
4. **Locality**: Enables cache efficiency and tiling (this chapter)
5. **Redundancy**: Enables compression and quantization
6. **Symmetry**: Enables weight sharing and equivariant architectures

Plus **Reversibility**, which enables memory-efficient training by trading recomputation for storage.

These aren't just theoretical curiosities. They're the building blocks of every efficient algorithm in this book.

In Part III, we'll see how these properties are wielded in practice. We'll investigate matrix multiply, FlashAttention, LoRA, quantization, and state-space models—watching how each exploits multiple properties in concert.
